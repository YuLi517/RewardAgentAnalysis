...
### 2.11a 团队培育奖金 4 档精确匹配 (PR #71 拍板, 2026-08-06)

- **业务**: 用户 (2026-08-06) 拍板"团队培育奖金按新成员 PV 分档, 不同档不同比率"
- **业务规则 (PR #71, 严格精确匹配)**:
...
- **新算法** (main.py._build_tree_from_db):
  - 旧 (PR #69): `teamBonus = (1区新PV + 2区新PV) × 30%` (单一比率)
...
- **业务存储 (DB JSON 字段)**:
  ...
  - 格式: `{"version": 1, "locks": {bfs_id: {"subs": [4 bfs_ids], "m_first": int}, ...}}`
  - 业务: 4 子关系 = scenario 树形属性, 跟 layer_counts 一样持久化
  - 旧 134 scenario lazy backfill: 首次 GET 触发 (repository.load 自动)
- **业务可视化**:
  - `tools/inspect_one_gen_four_locks.py [scenario_id]`: 全网列所有 lock 父节点
...
### 2.14 严格按 tree 公式范围设置全树 (2026-08-05 拍板, 10 个 line 3-5 节点)

- **业务**: 用户 (2026-08-05) 反馈 "严格按照公式范围, 点位的区间, 去设置目前的树. 不能有超过这个区间的数字存在."
- **当前问题**:
  - L2+ 节点 line 3-5 走 5 叉 own 公式, gnode = `10000 + 5^level + (parent_bfs*5 + (line-1))`
  - 这些 gnode 不在 tree 公式层范围内 (L4 应 30-61, 但 line 3-5 节点 gnode 10000+)
  - 共 10 个 line 3-5 节点需要调整 line 到 1 或 2
- **业务规则 (用户拍板)**: 所有 L2+ 节点 line 必须 ∈ {1, 2}, 走 tree 公式
  - line 3-5 节点全部 line 调整到 line 1/2 (按父的空闲槽)
  - gnode 严格在 tree 公式层范围内: L1=2-5, L2=6-13, L3=14-29, L4=30-61, L5=62-125, ...
  - L1 父 4 个 (line 1-4) 仍走 tree 公式 (BFS 0-3, gnode=2,4,3,5), 因为 L1 父是 root 直接 children
  - L0 root 不在范围内 (line=5 是 metadata, gnode=1 在 L0 范围)
- **DB 改动 (gitignored, 10 个节点)**:

  | dist_id | parent | old line | new line | old gnode | new gnode | 新 gnode 范围 |
  |---|---|---|---|---|---|---|
  | A9278335.1 | A8940454.1 | 5 | 1 | 10664 | **44** | L4 ∈ [30, 61] |
  | N5736507.1 | N5635585.1 | 3 | 2 | 10672 | **55** | L4 ∈ [30, 61] |
  | N5039236.1 | A9278335.1 | 5 | 1 | 13179 | **72** | L5 ∈ [62, 125] |
  | A8066781.4 | N5637604.1 | 3 | 2 | 25672 | **226** | L6 ∈ [126, 253] |
  | N5133878.1 | N5039236.1 | 5 | 2 | 25719 | **208** | L6 ∈ [126, 253] |
  | N7994055.1 | N5637650.1 | 3 | 1 | 400692 | **686** | L8 ∈ [510, 1021] |
  | N6094306.1 | N5135164.1 | 3 | 1 | 400792 | **642** | L8 ∈ [510, 1021] |
  | N8341149.1 | N6000671.1 | 3 | 2 | 1963287 | **1798** | L9 ∈ [1022, 2045] |
  | N7422208.1 | N6094306.1 | 5 | 2 | 1963294 | **1542** | L9 ∈ [1022, 2045] |
  | N8335868.1 | N7422208.1 | 3 | 1 | 9775782 | **2782** | L10 ∈ [2046, 4093] |
- **业务影响** (树形结构调整, 全树 264 节点重新 BFS 排序):
  - 10 个 line 3-5 节点改 line 后, 部分 L4-L8 父节点的 L_k+1 孩子重新 BFS 排, gnode 全部按 tree 公式
  - 例: N6094306.1 改 line 3→1 后, 它的 L8 bfs 变了, 下游 N7422208.1 / N8335868.1 的 bfs 也变了, 但全部按新 sort 重算, 全部落在 tree 范围内
- **验证结果**: 全部 264 节点 gnode 严格在层 tree 公式范围内, 0 不符
- **踩坑教训** (改前要 verify 下游 bfs):
  - 改一个节点的 parent_line_id 后, 它下面所有子孙的 bfs 都可能重排 (因为 sort by (parent_bfs, line))
  - 改前用 migration 算一次期望 gnode, 改后再算一次, 两次 expected gnode 可能完全不同
  - 我第一次 (在 Playwright 验证时) 用了 N6094306.1 改 line 前的 bfs=33 算下游 N8335868.1 expected=3038, 实际是 2782. 必须用改 line 后的 bfs
- 改 tree 公式必查 §5.32 + §5.34 列出的所有引用点

### 2.15 严格 L2+ 2 叉清理 (2026-08-06 拍板, 244 违规节点)

- **业务**: 用户 (2026-08-06) 截图反馈 N5146672.1 (喻传华) 有 3 个子 (A2128/A2129/A2130), 但业务规则说 L2+ 应严格 2 叉
  > 用户原话: "应该都严格按照2叉树来部署。只有L1层是4个。其他的层级，每个节点都是2个分支."
- **业务规则 (用户拍板)**:
  - L0 root: 4 大区 (line 1-4 children) — root 是唯一例外
  - L1 父 (root 的 4 大区): max_lines=4, 但实际只用 2 子 (line 1-2), L2 严格 2 叉
  - **L2+ 严格 2 叉** (line 1-2 only, line 3-5 全部删除)
- **当时违规情况** (snapshot 264 节点 + 2000 新 + 6 旧 = 2270 节点):
  - 244 个 L2+ 父 max_lines>2 (224 max_lines=5, 2 max_lines=4, 18 max_lines=3)
  - 4 个 L1 父 max_lines=5 (因 batch2000_v2 用 max_lines=5 加), 残留 line 3-5 子
  - 1 个 L1 父 (A8121400.1) max_lines=3 + 1 个 line 3 子 (A2010)
- **清理动作 (3 步)**:
  1. **删 L2+ 父 line 3+ 子 + 子孙 (1498 节点)**:
     - 找所有 L2+ 父 max_lines>2
     - 递归 CTE 找每个父的 line 3+ 子树
     - 删 ledger 1498 条 + members 1498 个
  2. **补 L1 父 line 3+ 子清理 (12 节点)**:
     - 4 个 L1 父全部改 max_lines=4 (跟 root 4 大区对齐)
     - 删 line 3+ 子 + 子孙: A2007/A2008/A2009 (A8077082.1) + A2010 (A8121400.1) + 8 个 L3 子孙
  3. **删 A2010 子树 (3 节点)**:
     - 第一次 cleanup 漏了 (A8121400.1 max_lines=3 不在 `>4` 判断内)
     - 补一刀: 删 A2010 + 2 个 L3 子
- **最终结果 (760 节点)**:
  - L0=1 (root), L1=4, L2=8, L3=16, L4=32, L5=56, L6=60, L7=108, L8=188, L9=177, L10=68, L11=34, L12=6, L13=2
...
- **关键脚本**:
  ...
  - `tools/cleanup_l1_line345.py` — L1 父 line 3-5 子清理 (12 节点)
  - 手动删 A2010 子树 (3 节点) — cleanup_l1_line345 没覆盖 max_lines=3 情况
- **DB 备份** (gitignored):
  - `data/rewarddb.db.bak-2026-08-06-pre-line3-cleanup` (2270 节点, 清理前)
- **踩坑教训**:
  - 清理 L1 父时**别用 `if p_ml > 4` 判断**, 因为有些 L1 父 max_lines=3/5 都有 line 3 子
  - 正确做法: 遍历所有 L1 父, 递归删 line 3+ 子 (跟 L2+ 一样的逻辑)
  - 我第一次 cleanup 漏了 A2010 (A8121400.1 max_lines=3 情况), 验证发现再补刀
  - **Rule**: 任何"line 3+ 子清理"脚本, 触发条件是 **parent 是不是 L1 (depth=1)**, 不是 parent 的 max_lines 值
- 改树形结构必查 §2.14 + §5.32 + §5.34 列出的所有引用点

### 2.16 L0-L9 全满 + L10 部分填充 (2026-08-06 拍板, 2144 节点)

- **业务**: 用户 (2026-08-06) 拍板要 "L0-L9 全满 + L10 部分填充", 合计 2144 节点
- **业务规则 (用户拍板)**:
  - L0-L9 严格 2^k 全满 (1+4+8+16+32+64+128+256+512+1024 = 2045)
  - L10 部分填充, 99/2048 (= 2144 - 2045)
  - 严格 2 叉 (L1=4, L2+=2)
- **当前结构 (2144 节点)**:
  | 层 | 节点 | 状态 |
  |---|---|---|
  ...
  | **合计** | **2144** | |
- **执行 (2 阶段)**:
  1. **添加 L7-L9 全满 (+896)**: L7 225→256 (+31), L8 240→512 (+272), L9 432→1024 (+592). 按"找父节点的空 slot, 填 line 1-2" BFS.
  2. **删除 L10+ (-895)**: 删 L11-L14 (242 节点) + L10 减到 99 (删 653 节点, 保留前 99 个 BFS order). 净 = -895.
- **净变化**: +896 -895 = +1 (2143 → 2144)
- **关键脚本**: `tools/layer_full_2144.py` (idempotent, 包含 2 阶段: 填 L7-L9 + 删 L10+)
- **DB 备份** (gitignored): `data/rewarddb.db.bak-2026-08-06-pre-L0-L9-full` (填充前 2143 节点, L10+ 残缺)
- **踩坑教训**:
  - "层层全满" 跟"合计 2144" 冲突, 2144 不是 2^k 总和, 必须 L10 部分填充
  - 添加时按"父的 空 slot" BFS, 不要按"广度 BFS" 否则 L7 填不满 (L7 父 128 × 2 = 256 slots, 但 L7 现有 224 = 一些 L6 没 2 子)
  - 删 L10 时按"parent_dist_id, slot_line_id" 排序, 保留前 99 个, 删除其余 653 (避免 SQL IN 子句过长)
  - **Rule**: 任何"层满" 校验, 都得先看 2^k 总和能不能 fit 目标, 不能 fit 就老实 partial 末层
- 改 tree 公式 / 层满填充必查 §2.14 + §5.32 + §5.34 列出的所有引用点
